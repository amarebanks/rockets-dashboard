...
# Houston Rockets Analytics Dashboard

A full-stack NBA analytics platform — advanced stats, a CBA-accurate trade and salary-cap engine, draft-capital valuation, and predictive models — built around the Houston Rockets and the entire league.
...
A full roster browser with a 2K-style overall rating for each player, plus deep individual profiles featuring league-ranked averages, advanced metrics (Offensive/Defensive Rating, True Shooting, eFG%, Usage, PIE), a shot chart, game-by-game trends, and a skill radar.
...
### Trade Machine
A cap-legal trade builder. Drag players from any two teams and instantly see salary in and out, value balance, and whether the deal is legal under the team's cap, tax, and apron situation — enforcing the simplified 2023 CBA salary-matching rules.
...
The contract and draft-pick scrapers are written with the Python standard library only (no third-party HTML parser), and bypass bot protection with realistic browser headers.
...
│   ├── scraper.py          NBA data pipeline — populates PostgreSQL
│   ├── trade_ideas.py      Trade value model + Championship Builder engine
│   ├── contracts.py        Salary-cap engine, CBA matching, cap-relief planner
│   ├── recognition.py      Player recognition tiers + accent-safe name matching
│   ├── draft.py            Draft-pick ownership, valuation, and pick numbers
│   ├── spotrac_scraper.py  Contract/cap scraper
│   ├── draft_scraper.py    Draft-pick scraper (all 30 teams)
│   └── .env.example        Environment variable template
├── frontend/
│   └── src/
│       ├── App.js          Routing and navigation
│       ├── pages/          One module per page (Dashboard, Players, Caps, ...)
...
| GET | `/compare` | Two-player comparison (via search + stats) |
